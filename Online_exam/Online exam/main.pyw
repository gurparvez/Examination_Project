...
def optionFrameGui(uid):
    ...
    global nameButton
    nameButton=CTkButton(studentNameFrame,text=exStudentName[0][0].capitalize(),font=("Roboto", 20 ,"bold"),command=lambda:studentInformationGui(uid),height=50,image=UserImage,fg_color='transparent',text_color='white')
...
def textFrameGui():
    # textLabel=CTkLabel(textFrame,text="",font=('Roboto',100))
    # textLabel.pack()
...
def ansSubmit(var,uid,qId,sId,permission=False):
    # senddata-----------------------------------
    if not permission:
...
def studentInformationGui(uid):
    ...
    if "studentInformationFrame" not in packFrame:
        ...
        nameLabel=CTkLabel(studentInformationFrame,text=f'{exInformation[0][1].capitalize()}',font=('Roboto',50))
...
def editProfile(uid):
    ...
    editFarme.pack(pady=(50,0),padx=10)
...
def profileSubmit(name1,name2,email,password,uid):
    ...
    if check:
        ...
        nameButton.configure(text=f"{name1} {name2}")
...
def aboutGui():
    ...
    if "aboutFrame" not in packFrame:
        ...
        for widget in aboutFrame.winfo_children(): #destroy old widget--
            widget.destroy()
        label=CTkLabel(aboutFrame,text="about",font=("Roboto", 40)).pack()
